...
module.exports = function (source) {
  ...
  if (typeof config.excludeGlobals === "string") exclGlobals = [exclGlobals];

  function replacer(match, varName, pkgName) {
    if (config.exclude[pkgName]) return "";

    if (Array.isArray(exclGlobals))
      if (
        exclGlobals.indexOf(pkgName) > -1 ||
        exclGlobals.indexOf(varName) > -1
      )
        return "";
    return match;
  }
