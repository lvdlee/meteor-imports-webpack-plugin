...
function readPackages(callback) {
  ...
  this.fs.readJson(programJsonPath, (err, json) => {
    if (err)
      return callback(
        new Error(
          `Unable to read ${programJsonPath}. This file should be generated by Meteor.` +
    ...
    const packages = json.manifest
      ...
      .map((x) => {
        ...
        if (excludeEntry === true) return null;
        ...
        if (typeof excludeEntry === "object") {
          ...
          if (excludeEntry.mode === mode) return true;
        }
  ...
  readPackages.call(this, (err, packages) => {
    ...
    for (let pkg of packages) {
      if (pkg.source)
        ...
        output +=
          "require(" +
          JSON.stringify(path.join(meteorBuild, pkg.path)) +
          ");\n";
    }
