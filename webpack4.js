...
const id = "meteor-imports-plugin";
const NS = "meteor-imports";
...
MeteorImportsPlugin.prototype.apply = function (compiler) {
  ...
  if (compiler.hooks) {
    // webpack 4
    compiler.hooks.compile.tap(id, (params) => {
      ...
      compiler.resolverFactory.plugin("resolver [context]", (resolver) => {
        new AliasPlugin(
          "described-resolve",
          {
      ...
      compiler.resolverFactory.plugin("resolver [loader]", (resolver) => {
        new ModulesInRootPlugin("module", meteorNodeModules, "resolve").apply(
          resolver
        );
      });
      ...
      // globals.
      var excluded = new RegExp(
        self.config.exclude
          .map(function (exclude) {
            return "^packages/" + exclude + ".js$";
          })
          .concat("^app/.+.js$")
          .join("|")
      );
      ...
      manifest.forEach(function (pckge) {
        if (!excluded.test(pckge.path)) {
          ...
          extraRules.push({
            meteorImports: true,
            test: new RegExp(
              ".meteor/local/build/programs/web.browser/" + pckge.path
            ),
    ...
    compiler.plugin("compile", (params) => {
      ...
      compiler.resolvers.loader.apply(
        new ModulesInRootPlugin("module", meteorNodeModules, "resolve")
      );
